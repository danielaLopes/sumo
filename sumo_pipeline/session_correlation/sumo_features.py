...
#def write_headers_cv_files(arff_stats, arff_pl):
def get_headers():
    f_names_stats = []
    ...
    return f_names_stats, f_names_pl
...
class SessionFeatures:
    ...
    machine_ip : str
    timeSamplingInterval: int # TODO: so far this isn't doing anything, this is to be used eventually when we bucketize the packets here, however it might not be the best option for parameter flexibility
    special_features = dict[str, object]
    ...
    def __init__(self, timeSamplingInterval):
        self.first_ts = 0
        self.last_ts = 0
        self.packetTimes = []
        self.packetTimesIn = []
...
class SumoFeatures:
    ...
    def __init__(self, timeSamplingInterval, dataset_name, datasetPath):
        self.pairsFolders = {}
        self.timeSamplingInterval = timeSamplingInterval
    ...
    def process_alexa_features(self, session_path):
        ...
        session_features = SessionFeatures(self.timeSamplingInterval)
        try:
        ...
        try:
            f_values_stats, f_values_pl = session_features.get_statistical_features(session_path, LABEL_IS_CLIENT_SIDE, session_path.split('/')[-1])
        ...
        return f_values_stats, f_values_pl, LABEL_IS_NOT_SESSION_TO_OS


    def extract_alexa_features_parallel(self, alexaCaptures, arff_path_stats_source, arff_path_pl_source):
        print("\n--- ALEXA")
    #def extract_alexa_features_parallel(self, alexaCaptures):
        ...
        f_values_pl = [result[1] for result in results if result is not None]
        ...
        pl_source_pd_alexa = pd.DataFrame(f_values_pl)
        pl_source_pd_alexa.to_csv(arff_path_pl_source, mode='a', index=False, header=False)
    ...
    def process_client_features(self, session_path):
        ...
        return f_values_stats, f_values_pl, LABEL_IS_SESSION_TO_OS

    
    def extract_client_features_parallel(self, clientCaptures, arff_path_stats_source, arff_path_pl_source, arff_path_stats_target, arff_path_pl_target, labels_target_alexa):
        print("\n--- CLIENT")
        ...
        pl_source_pd_client = pd.DataFrame(f_values_pl)
        pl_source_pd_client.to_csv(arff_path_pl_source, mode='a', index=False, header=False)
        ...
        pl_target_pd = pd.read_csv(arff_path_pl_source, low_memory=False)
        ...
        pl_target_pd['Class'] = labels_series

        #print("stats_target_pd['Capture']", stats_target_pd['Capture'])
        #print("pl_target_pd['Capture']", pl_target_pd['Capture'])

         # Write target separation features with only the alexa and client features
        stats_target_pd.to_csv(arff_path_stats_target)
        pl_target_pd.to_csv(arff_path_pl_target)
    ...
    def process_onion_features(self, session_path):
        ...
        try:
            f_values_stats, f_values_pl = session_features.get_statistical_features(session_path, LABEL_IS_OS_SIDE, session_path.split('/')[-1])
        except StatisticalFeaturesException:
            print("StatisticalFeaturesException")
            traceback.print_exc()
            return None
    ...
    def extract_onion_features_parallel(self, onionCaptures, arff_path_stats_source, arff_path_pl_source):
        print("\n--- ONIONS")
    #def extract_onion_features_parallel(self, onionCaptures):
        ...
        pl_source_pd_onion = pd.DataFrame(f_values_pl)
        pl_source_pd_onion.to_csv(arff_path_pl_source, mode='a', index=False, header=False)
    ...
    def extract(self, alexaCaptures, clientCaptures, onionCaptures):
        ...
        arff_path_pl_source = f'{self.topPath}/pl_source_separation.csv'

        arff_path_stats_target = f'{self.topPath}/stats_target_separation.csv'
        arff_path_pl_target = f'{self.topPath}/pl_target_separation.csv'

        stats_columns, pl_columns = get_headers()
        stats_source_pd = pd.DataFrame(columns=stats_columns)
        pl_source_pd = pd.DataFrame(columns=pl_columns)
        stats_source_pd.to_csv(arff_path_stats_source, mode='w', index=False)
        pl_source_pd.to_csv(arff_path_pl_source, mode='w', index=False)

        labels_target = self.extract_alexa_features_parallel(alexaCaptures, arff_path_stats_source, arff_path_pl_source)
        self.extract_client_features_parallel(clientCaptures, arff_path_stats_source, arff_path_pl_source, arff_path_stats_target, arff_path_pl_target, labels_target)
        self.extract_onion_features_parallel(onionCaptures, arff_path_stats_source, arff_path_pl_source)
        #labels_target = self.extract_alexa_features_parallel(alexaCaptures)
        #self.extract_client_features_parallel(clientCaptures, arff_path_stats_target, arff_path_pl_target, labels_target)
        #self.extract_onion_features_parallel(onionCaptures)

        #self.stats_source_pd.to_csv(arff_path_stats_source)
        #self.pl_source_pd.to_csv(arff_path_pl_source)
